...
def worker(grid, sim, n, lock):
    while sim.t < sim.t_end:
        ...
        # sim.t += sim.dt  # Das Inkrement von sim.t sollte innerhalb des kritischen Abschnitts erfolgen

        grid.u, grid.v, grid.p = grid.u.copy(), grid.v.copy(), grid.p.copy()
        n.value += 1  # Use Value to share 'n' between processes
        sim.t += sim.dt  # Inkrement von sim.t nach dem kritischen Abschnitt
        lock.release()
...
if __name__ == '__main__':
    ...
    for _ in range(num_processes):
        process = multiprocessing.Process(target=worker, args=(grid, sim, n, lock))
