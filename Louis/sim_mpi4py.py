...
def mpi_worker(sim, grid, rank, size):
    n = 0
    while sim.t < sim.t_end:
        n = 0
        select_dt_according_to_stability_condition(grid, sim)
        print(f"Rank {rank}, t: {sim.t:.3f} dt: {sim.dt} res: {sim.res}")
...
def mpi_simulation(sim, grid):
    ...
    local_imax = grid.imax // size
    local_grid = StaggeredGrid(local_imax, grid.jmax, grid.xlength, grid.ylength)

    comm.Scatter([grid.u, MPI.DOUBLE], [local_grid.u, MPI.DOUBLE])
    comm.Scatter([grid.v, MPI.DOUBLE], [local_grid.v, MPI.DOUBLE])
    comm.Scatter([grid.p, MPI.DOUBLE], [local_grid.p, MPI.DOUBLE])

    mpi_worker(sim, local_grid, rank, size)

    comm.Gather([local_grid.u, MPI.DOUBLE], [grid.u, MPI.DOUBLE])
    comm.Gather([local_grid.v, MPI.DOUBLE], [grid.v, MPI.DOUBLE])
    comm.Gather([local_grid.p, MPI.DOUBLE], [grid.p, MPI.DOUBLE])
...
if __name__ == "__main__":
    ...
    mpi_simulation(sim, grid)

    # Rank definition ist noch nicht ganz klar
    ...
    if rank == 0:
        save_matrix("u.dat_mpi4py", grid.u)
        save_matrix("v.dat_mpi4py", grid.v)
        save_matrix("p.dat_mpi4py", grid.p)
